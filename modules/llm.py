...
class MeetingMinutesService:
    ...
    def _generate_with_openai(self, input_data: MeetingMinutesInput) -> str:
        ...
        response = client.chat.completions.create(
            model=input_data.model,  # 入力データから取得
            messages=[
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": user_prompt}
            ],
            max_tokens=input_data.max_tokens,  # 入力データから取得
            temperature=input_data.temperature,  # 入力データから取得
            top_p=input_data.top_p  # 入力データから取得
        )
